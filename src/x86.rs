...
impl Unwind {
    ...
    fn unwind_next(&mut self) -> bool {
        unsafe {
            let stack: *const StackFrame = self.fp as *const StackFrame;
            self.fp = (*stack).fp;
            ...
            // go to next ra
            let stack: *const StackFrame = self.fp as *const StackFrame;
            self.pc = (*stack).ra.saturating_sub(0x4);
            if self.pc == 0 {
                debug!("pc: {:#016X}  sp={:#016X}", (*stack).ra, self.fp);
                return false;
            }
        }
...
fn current_unwind() -> (usize, usize) {
    unsafe {
        let mut fp: usize;
        asm!("mov {ptr}, rbp", ptr = out(reg) fp);
        let stack: *const StackFrame = fp as *const StackFrame;
        ((*stack).ra - 0x4, fp)
    }
}
